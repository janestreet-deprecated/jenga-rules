...
let failheref : Lexing.position -> ('a, unit, string, unit -> 'b) format4 -> 'a =
  fun here fmt ->
    let col = here.pos_cnum - here.pos_bol in
    Located_error.raisef
      ~loc:{ source = File (root_relative "jenga/root.ml")
           ; line = here.pos_lnum
           ; start_col = col
           ; end_col = col
           }
...
module Alias = struct
  ...
  let enforce_style ~dir = Alias.create ~dir "enforce-style"
...
let javascript_enabled =
  Var.peek_register_bool "WITH_JAVASCRIPT"
    ~default:(Compiler_selection.m32 || Config.public)

let javascript_separate_compilation = for_javascript_development

(* If [PORTABLE_INT63] is [false], then [Base.Int63.t] is exposed as [= private int] on
   64-bit systems. This enable extra compiler optimizations.

   However, the generated byte-code is no longer compatible with 32-bit platforms. With
   [PORTABLE_INT63=true], the type [Base.Int63.t] is selected at runtime and so the
   byte-code generated for [Base.Int63] remains portable. *)
let portable_int63 =
  Var.peek_register_bool "PORTABLE_INT63"
    ~default:(javascript_enabled ||
              (match build_profile with
               | `Fast_build | `Default -> true
               | `Fast_exe -> false))
...
(* Where external libraries are imported *)
let external_lib_dir = Path.root_relative "external"
...
module Libmap : sig
  ...
  let resolve_libdep_name_exn t dep =
    ...
    | Some x -> x
    | None -> failheref [%here] !"unknown library %{Libdep_name}" dep ()
...
module User_or_gen_config : sig
  ...
  let load ~dir =
    ...
    | true ->
      ...
      *>>| fun contents ->
      let jbuilds =
        Sexp.many_of_string_conv_exn ~source:(File jbuild) contents
          [%of_sexp: Jbuild_with_if.t filtered]
        |> List.concat
      ...
      in
      let in_external = Path.is_descendant ~dir:external_lib_dir dir in
      let jbuilds =
        match List.exists jbuilds ~f:(function `enforce_style _ -> true | _ -> false) with
        | true -> jbuilds
        | false ->
          if in_external
          then Located_error.raisef ~loc:(dummy_position (source_file ~dir))
                 !"Every jbuild below %{Path} must specify whether to enforce_style.  \
                   Usually, the right choice is to opt out: \
                   %{sexp:Jbuild.Sexp_of.t}."
                 external_lib_dir
                 (`enforce_style { Enforce_style_conf.default with enabled = false }) ();
          `enforce_style Enforce_style_conf.default :: jbuilds
      in
      if not in_external &&
         List.exists jbuilds ~f:(function
           | `library { name; _} -> LN.is_flat name
           | _ -> false)
      then
        Located_error.raisef ~loc:(dummy_position jbuild)
          !"'_flat' libraries are not allowed outside of %{Path}"
          external_lib_dir ()
      else
        jbuilds
  ;;
    ...
    List.concat_map libs ~f:(function
      | Lib_dep.In_the_tree lib ->
        let flags =
          [ "-I"; reach_from ~dir (In_the_tree.liblink_dir ~libname:lib.name) ]
        in
        (* In theory these -open are problematic because they can shadow the libraries in
           further -open, including the -open for the library being compiled. In practice,
           this should be used by a limited number of library and if the library is not
           wrapped externally, it should have relatively unique names, making the problem
           seem not very likely. If the external library does the trick of not installing
           some cmis, we can always write a _flat module by hand to hide the
           corresponding modules. *)
        if LN.is_flat lib.name
        then flags @ ["-open"; LN.to_module lib.name]
        else flags
      | From_compiler_distribution compiler_lib -> begin
          match From_compiler_distribution.search_path_dir compiler_lib with
          | None -> []
  ...
  Rule.create ~targets (
    ...
    *>>| fun actual_dependencies ->
    ...
    | (bad_bn, _) :: _ as errors ->
      ...
      Located_error.raisef
        ~loc:(dummy_position (BN.suffixed ~dir name suf))
...
let staged_link (module Mode : Ocaml_mode.S) (dc : DC.t) ~dir
  ...
  let js_of_ocaml_rules =
    match Mode.which, js_of_ocaml with
    | `Byte, Some { Js_of_ocaml_conf.flags; javascript_files; toplevel} ->
      ...
      let toplevel = match toplevel with
        | [] -> None
        | _ :: _ ->
          let libs = Libmap.resolve_libdep_names_exn dc.libmap toplevel in
          let cmi_deps = LL.dep_on_ocaml_artifacts libs ~suffixes:[".cmi"] in
          Some (libs,cmi_deps)
      in
...
let inline_test_runner_env_clearing_directives =
  [ (* We export TZ so that tests do not depend on the local timezone. *)
    ("TZ", "America/New_York")
  (* The rest are exported this way in run-tests.py. In the past, when
     the local was not being set, inline test runner and jenga would
     have different outputs for things that cared about locale.
  *)
  ; ("TERM"    , "xterm")
  ; ("LANG"    , "C")
  ; ("LC_ALL"  , "C")
...
let inline_tests_script_rule =
  let define_inline_tests_env =
    String.concat
      (List.map inline_test_runner_env_clearing_directives ~f:(fun (k,v) ->
         sprintf !"export %{quote}=%{quote}\n" k v))
  in
  fun ~uses_catalog ~dir ~libname ~runtime_environment ~script:target ~flags ->
    let run =
      if drop_test
...
let run_inline_action ~dir ~uses_catalog ~user_deps ~exe_deps ~flags ~runtime_deps
      ~timeout ~sandbox ~for_javascript filename =
  ...
  *>>| fun () ->
  ...
  let timeout =
    match timeout with
    | None -> Inline_tests.default_timeout ~for_javascript
...
let bench_runner_cmx =
  ...
  if Compiler_selection.m32 || Config.public
...
let scheme_of_generated_dirs ~dir : Env.Per_directory_information.t option =
  let parent_dir = dirname dir in
  if Path.(=) dir Boot.parent_dir then
    Some { scheme = Boot.setup_parent_dir ~dir
         ; directories_generated_from = Some Boot.parent_dir }
  else if Path.(=) parent_dir Boot.parent_dir then
    Some { scheme = Boot.setup_dir ~dir
         ; directories_generated_from = Some Boot.parent_dir }
  else if Path.(=) parent_dir LL.dir then
    Some { scheme = setup_liblinks ~dir
         ; directories_generated_from = Some LL.dir }
  else if Path.(=) dir LL.dir then
    Some { scheme = setup_liblinks_dir ~dir
         ; directories_generated_from = Some LL.dir }
    ...
    Some { scheme = setup_odoc ~dir `Compile
         ; directories_generated_from = Some Odoc.odoc_output_dir
         }
  else if Path.(=) parent_dir Odoc.html_output_dir then
    Some { scheme = setup_odoc ~dir `Html
         ; directories_generated_from = Some Odoc.html_output_dir
         }
...
let scheme ~dir : Env.Per_directory_information.t =
  ...
  then { scheme = Scheme.empty; directories_generated_from = None }
  else
